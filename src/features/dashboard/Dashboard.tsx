...
export default function Dashboard() {
  ...
  return (
    <div className="p-4 space-y-6">
      <h2 className="text-xl font-semibold text-slate-800">대시보드</h2>
          ...
          {(['day', 'week', 'month'] as const).map((p) => (
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-sm font-medium ${
                period === p ? 'bg-primary text-white' : 'bg-slate-200 text-slate-700'
              }`}
            >
      ...
      <section className="grid grid-cols-1 gap-3">
        <h3 className="text-sm font-medium text-slate-600 col-span-full">전체 소득 · 지출</h3>
        <div className="rounded-xl bg-emerald-50 border border-emerald-200 p-4">
          <p className="text-xs text-emerald-700 mb-1">총 소득</p>
        ...
        <div className="rounded-xl bg-red-50 border border-red-200 p-4">
          <p className="text-xs text-red-700 mb-1">총 지출</p>
          <p className="text-slate-500 text-sm">
        ...
        <div className="rounded-xl bg-slate-100 p-4 col-span-full">
          <p className="text-xs text-slate-600 mb-1">순계 (기준 통화)</p>
      ...
      <section>
        ...
        {isLoading ? (
          ...
          <div className="h-64 w-full rounded-lg bg-white border border-slate-200 p-2">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                ...
                <Bar dataKey="지출_원" fill="#ef4444" name="지출 (원)" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
